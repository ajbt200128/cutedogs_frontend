...
export class SignupComponent implements OnInit {
  ...
  getUsernameError(): string {
    if (this.formGroup.controls['usernameValidator'].errors) {
      if (this.formGroup.controls['usernameValidator'].errors['required']) {
        ...
        this.formGroup.controls['usernameValidator'].errors['pattern']
      ) {
        return 'Username must be alphanumerical and 8-20 characters long';
      } else if (
        this.formGroup.controls['usernameValidator'].errors['usernameTaken'] ||
        !this.valid
      ) {
  ...
  getPasswordError(): string {
    if (this.formGroup.controls['passwordValidator'].errors) {
      if (this.formGroup.controls['passwordValidator'].errors['pattern']) {
        ...
        this.formGroup.controls['passwordValidator'].errors['required']
      ) {
  ...
  checkPassword() {
    ...
    if (!validP) {
      this.formGroup.controls['passwordVerifyValidator'].setErrors({
        matching: true
      });
    } else {
