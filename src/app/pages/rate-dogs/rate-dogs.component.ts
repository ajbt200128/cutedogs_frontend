...
export class RateDogsComponent implements OnInit {
  ...
  like(){
    if (LoginHelper.isLoggedIn()){
      ...
      this.api.likeImage(this.dogs[this.currDog].images[this.currImg].uuid,
        (this.dogs[this.currDog].images[this.currImg].likedBy.indexOf(LoginHelper.getProfile().uuid) <= -1)).subscribe(
          res=>{
            (this.dogs[this.currDog].images[this.currImg].likedBy.push(LoginHelper.getProfile().uuid));
            this.next();
        });
  ...
  next(){
    this.currDog++;
    this.currImg = Math.floor(Math.random() * this.dogs[this.currDog].images.length)
    console.log(this.dogs[this.currDog].images[this.currImg]);
    if (this.currDog == this.dogs.length - 1){
      this.api.getDogsRandom(5).subscribe(res => this.dogs = this.dogs.concat( res.data));
    }
  }
  tagClick(tag:string){
